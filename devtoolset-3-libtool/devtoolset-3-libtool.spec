...
BuildRoot: %{_tmppath}/%{pkg_name}-%{version}-%{release}-%(%{__id_u} -n)
Requires(post):  /sbin/install-info
Requires(preun): /sbin/install-info

%{?scl:Requires: %{scl}-runtime}
%{?scl:BuildRequires: %{scl}-runtime}
BuildRequires: %{?scl_prefix}autoconf >= 2.59, %{?scl_prefix}automake >= 1.9.2
#BuildRequires: %{?scl_prefix}texinfo
BuildRequires: texinfo
Requires: %{?scl_prefix}autoconf >= 2.58, %{?scl_prefix}automake >= 1.4
...
Provides: %{?scl_prefix}%{name}-libs = %{version}-%{release}
License:  LGPLv2+
...
Requires: %{?scl_prefix}%{name}-ltdl = %{version}-%{release}
License:  LGPLv2+
...
/opt/%{?scl_prefix}/root/usr/share/info/libtool.info*
%{_bindir}/libtool
%{_bindir}/libtoolize
%{_datadir}/aclocal/*.m4
