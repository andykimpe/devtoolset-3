%{?scl:%scl_package automake14}
%{!?scl:%global pkg_name %{name}}

%define tarname automake-1.4-p6

Summary:    A GNU tool for automatically creating Makefiles
Name:       %{?scl_prefix}automake14
Version:    1.4p6
Release:    19.2%{?dist}
License:    GPLv2+
...
Conflicts:  %{?scl_prefix}automake < 1.5
...
%{?scl:scl enable %{scl} - << \EOF}
set -ex
/sbin/install-info %{_infodir}/%{pkg_name}.info %{_infodir}/dir || :
%{?scl:EOF}
...
set -ex
if [ "$1" = 0 ]; then
    /sbin/install-info --delete %{_infodir}/%{pkg_name}.info %{_infodir}/dir || :
fi
%{?scl:EOF}
